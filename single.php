...
<?php if (get_field('add-youtube-embed') == true): ?>
  <style> #logo, #strap, h2, .custom-image-head{ visibility:hidden; } @media all and (max-width: 650px) {  #video-player:not(#cover-image){display:none} #background:not(#cover-image){display:none} } </style>

  <?php if (get_field('show-header-image') != true): ?>
    <section id="cover-image">
  <?php else: ?>
    <style>#logo, #strap, h2, .custom-image-head{ position: absolute; overflow: hidden; clip: rect(0 0 0 0); height: 1px; width: 1px; margin: -1px; padding: 0; border: 0;}</style>
  <?php endif?>
<?php endif ?>

<?php while ( have_posts() ) : the_post(); ?>
  <?php if (get_field('show-header-image') == true && (get_field('add-youtube-embed') != true)): ?>
    <img id="background-custom"
  <?php else: ?>
    <img id="background" src="<?php if (get_field('show-header-image') == true) {the_field('header-image-location'); } elseif (get_field('header-image-location') == false ) {echo site_url(); echo get_option('imageurl'); } ?>">
  <?php endif ?>
  ...
  <?php if (get_field('add-youtube-embed') != false): ?>
    <div id="video-player">
      <div class="embed-container">
        <iframe width="100%" height="100%" src="http://www.youtube.com/embed/'.get_field('youtube-video-uri').'?modestbranding=1&rel=0" frameborder="0"></iframe>
      </div>
    </div>
  <?php endif ?>
...
<section id="main">
  <article class="single-article" id="single-article">
    <?php
      if (get_field('have-audio-player') == true) {
        $location = get_field("audio-ogg-location");
        echo do_shortcode('[audio src="'.($location).'" preload="true"]');
      }
    ?>
    ...
    <div id="mobile-audio">
      <?php
        if (get_field('have-audio-player') == true) {
          $location = get_field("audio-ogg-location");
          echo '<audio src="'.($location).'" preload="auto" controls></audio>';
        }
      ?>
    </div>

    <?php
      if (get_field('add-youtube-embed') == true or (get_field('header-title') == false))
        echo "<h3 class=\"youtube-title\">".get_the_title($ID)."</h3>";
    ?>
